...
export function ArticleForm({ initialValues, onSubmit, submitLabel }: Props) {
  ...
  async function handlePickImage() {
    ...
    if (!permission.granted) {
      setError('Gallery access permission is needed to pick an image');
      return;
    }
    ...
    <ScrollView className="flex-1 bg-white dark:bg-gray-900" contentContainerClassName="p-4 pb-10">
      ...
      >
        {isUploadingImage ? (
          ...
          <Text className="text-gray-400 dark:text-gray-500">Tap to pick an image</Text>
        )}
      </Pressable>
      ...
      <Pressable
        ...
        className="items-center rounded-lg bg-emerald-600 py-3 disabled:opacity-50"
      >
        {isSubmitting ? (
